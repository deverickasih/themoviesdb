...
class APICall {
    ...
    func fetchReviews(movieId: Int, type: String = "movie", completionHandler: @escaping ([ReviewModel], Error?) -> Void) {
        
        let url = URL(string: "\(APIConstants.baseUrl)/\(type)/\(movieId)/reviews?api_key=\(APIConstants.apiKey)")!
    ...
    func fetchTrailers(movieId: Int, type: String = "movie", completionHandler: @escaping ([TrailerModel], Error?) -> Void) {
        
        let url = URL(string: "\(APIConstants.baseUrl)/\(type)/\(movieId)/videos?api_key=\(APIConstants.apiKey)")!
        
        let task = URLSession.shared.dataTask(with: url, completionHandler: { (data, response, error) in
            if let error = error {
                print("Error with fetching trailers: \(error)")
                completionHandler([], error)
            }

            if let data = data,
               let trailers = try? JSONDecoder().decode(TrailerList.self, from: data) {
                print("hey jan \n\(trailers.items)")
                completionHandler(trailers.items, nil)
            }else{
                let jsonString = String(data: data ?? Data(), encoding: .utf8)!
                print("hey man\n\(jsonString)")
                completionHandler([], nil)
